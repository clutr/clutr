...
from envs.wrappers import ParallelAdversarialVecEnv, VecMonitor, VecNormalize, \
    VecPreprocessImageWrapper, VecFrameStack, MultiGridFullyObsWrapper, CarRacingWrapper, TimeLimit, VecPreprocessUEDMultigridWrapper
...
def _make_env(args):
    ...
    #TOOD: multigrid exp
    if args.use_latent_task:
        env_kwargs.update({
                           'use_latent_task': args.use_latent_task,
                           'latent_task_exp': args.latent_task_exp,
                            'task_ae': args.task_ae,
                            'ae_type': args.ae_type,
                           'latent_space_discrete_max': args.latent_space_discrete_max,
...
def create_parallel_env(args, adversary=True):
    ...
    if is_multigrid:
        #used for multigrid env, i.e. not adversary
        obs_key = 'image'
        scale = 10.0

        if is_clutr:
            #ued_venv = VecPreprocessWrapper(venv=venv)
            ued_venv = VecPreprocessUEDMultigridWrapper(venv=venv)
        else:
            pass
    ...
    if not is_clutr and is_multigrid:
        ued_venv  = venv
